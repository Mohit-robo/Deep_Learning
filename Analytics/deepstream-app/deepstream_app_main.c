...
static void load_postproc_lib() {
    if (!postproc_lib_handle) {
        postproc_lib_handle = dlopen("/workspace/configs_dfine_87_classes/deepstream_python_anpr/custom_parsers/postprocess/libpostprocess.so", RTLD_LAZY);
        if (postproc_lib_handle) {
    ...
    NvDsBatchMeta * batch_meta, guint index)
{
  ...
  // --- Append our Plate Post-Processing Logic ---
  load_postproc_lib();

  for (NvDsMetaList * l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next) {
    NvDsFrameMeta *frame_meta = (NvDsFrameMeta *)l_frame->data;

    // First pass: Collect all plates
    ...
    for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      ...
      if (obj->class_id == 80 || obj->class_id == 83 || obj->class_id == 84) {
        if (num_plates < MAX_PLATES) {
          plates[num_plates++] = obj;
        }
      }
    }

    // Second pass: Collect words and map to best plate
    ...
    for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      ...
      if (obj->unique_component_id == 2) { // CRAFT objects
        ...
        for (int i = 0; i < num_plates; i++) {
          ...
          float px1 = p->rect_params.left;
          float py1 = p->rect_params.top;
          float px2 = px1 + p->rect_params.width;
          float py2 = py1 + p->rect_params.height;

          if (cx >= px1 - 5 && cx <= px2 + 5 && cy >= py1 - 5 && cy <= py2 + 5) {
            best_plate_id = i;
            break;
          }
        }
    ...
    if (process_plate_fn && free_plate_result_fn) {
      for (int i = 0; i < num_plates; i++) {
        ...
        for (int j = 0; j < num_words; j++) {
          ...
          // Parseq string is in classifier meta
          ...
          c_polys[j][0] = left;
          c_polys[j][1] = top;
          c_polys[j][2] = left + width;
          c_polys[j][3] = top;
          c_polys[j][4] = left + width;
          c_polys[j][5] = top + height;
          c_polys[j][6] = left;
          c_polys[j][7] = top + height;
        }
        ...
        if (res) {
          ...
          if (res->num_confs > 0) {
            float sum_conf = 0.0f;
            for (int k = 0; k < res->num_confs; k++) {
              sum_conf += res->confs[k];
            }
            avg_final = sum_conf / res->num_confs;
          }

          NvDsObjectMeta *p = plates[i];
          char plate_id[64];
          snprintf(plate_id, sizeof(plate_id), "%d_%d_%d_%d",
                   (int)p->rect_params.left, (int)p->rect_params.top,
                   (int)p->rect_params.width, (int)p->rect_params.height);

          g_print("Plate ID %s Final OCR Result: %s (conf: %.2f, status: %s)\n",
                  plate_id, res->text, avg_final, res->status);

          // User requested: "Currently print on terminal"
          // So we don't modify the display_text
