...
char* run_count;
char person_parent_path[FILE_NAME_SIZE];
// char face_parent_path[FILE_NAME_SIZE];
guint gpuId;
NvDsObjEncCtxHandle enc_ctx_handle;
...
gie_crop_img_probe (GstPad * pad, GstPadProbeInfo * info,
    gpointer u_data)
{
  ...
  for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
      l_frame = l_frame->next) {
    NvDsFrameMeta *frame_meta = (NvDsFrameMeta *) (l_frame->data);
    if (!g_hash_table_contains(appCtx->sensorInfoHash, frame_meta->source_id + (gchar*)NULL))
      continue;
    ...
    for (l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      ...
      //parse_pose_from_meta(frame_meta, obj_meta, ctx, ip_surf );
      // if (obj_meta->unique_component_id == PRIMARY_DETECTOR_UID){
      // if (obj_meta->class_id == PGIE_CLASS_ID_PERSON ) { 
      if (is_class_id_valid(obj_meta->class_id)) {
        //printf("\n source id : %d \n", frame_meta->source_id);
        //printf("\n cam : %s \n", config->sensor_id_list[frame_meta->source_id]);
        //printf("Saving image for track id: %ld \n", obj_meta->object_id);
        //snprintf(fileFrameNameString, FILE_NAME_SIZE,"/home/griffyn/Documents/sentinel_ai/person_crops/PERSON-%d_%d_%ld_%d_%d",frame_meta->source_id, frame_meta->frame_num, obj_meta->object_id, obj_meta->rect_params.width, obj_meta->rect_params.height);
        ...
        objData.scaledWidth = 200;
        objData.scaledHeight = 200;
        objData.isFrame = TRUE;
        //printf("File Name: %s \n",objData.fileNameImg);
        char save_path[FILE_NAME_SIZE];
        //g_snprintf(save_path, FILE_NAME_SIZE, "%s/%d_Person-%ld_%d%02d%02d_run%s", person_parent_path, gpuId+1, obj_meta->object_id, current_time->tm_year+1900, current_time->tm_mon+1, current_time->tm_mday, run_count);
        g_snprintf(save_path, FILE_NAME_SIZE, "%s/%s", person_parent_path, get_sensor_info(appCtx, frame_meta->source_id)->sensor_id );
        
        //printf("\nSource ID: %d \t sensor-id: %s\n", frame_meta->source_id, save_path);
        if (stat(save_path, &st) == -1) {
          mkdir(save_path, 0700);
        }
        g_snprintf(objData.fileNameImg, FILE_NAME_SIZE,"%s/%d_full.jpg", save_path, frame_meta->frame_num);
        objData.quality = 100;
        nvds_obj_enc_process (enc_ctx_handle, &objData, ip_surf, obj_meta, frame_meta);
        break;
      }
      // }
    }
...
create_processing_instance (AppCtx * appCtx, guint index)
{
  ...
  if (config->use_nvmultiurisrcbin){
    sensor_id = config->sensor_id_list[index];
  }
    ...
    bbox_generated_callback bbox_generated_post_analytics_cb)
{
  ...
  *sink_elem = *src_elem = NULL;
  gpuId = config->primary_gie_config.gpu_id;
  enc_ctx_handle = nvds_obj_enc_create_context (gpuId);
  if (!enc_ctx_handle) {
  ...
  if (*src_elem) {
    ...
    // NVGSTDS_ELEM_ADD_PROBE (pipeline->
    //     common_elements.primary_bbox_buffer_probe_id,*src_elem, "src",
    //     gie_crop_img_probe, GST_PAD_PROBE_TYPE_BUFFER,
    //     &pipeline->common_elements);
...
static char* get_run_count(const char* filepath) {
    FILE* file = fopen(filepath, "r");
    if (!file) {
        printf("Error opening file: %s\n", filepath);
        return "1";  // Default value if file can't be opened
    }

    static char run_count[32];  // Buffer for storing line
    while (fgets(run_count, sizeof(run_count), file)) {
        if (run_count[0] == '#' || run_count[0] == '\n') continue;

        // Remove trailing newline if present
        run_count[strcspn(run_count, "\n")] = '\0';

        char* delimiter_pos = strchr(run_count, '=');
        if (delimiter_pos) {
            *delimiter_pos = '\0';  // Null-terminate the key
            if (strcmp(run_count, "RUN_COUNT") == 0) {
    ...
    fclose(file);
    return "1";  // Return default value if RUN_COUNT is not found
}
...
{
  // run_count = getenv("RUN_COUNT");
  run_count = get_run_count("/opt/reid/.env");

  if (run_count[0] == '\0')
    run_count = "1";
  time_t t = time (NULL);
  current_time = localtime(&t);
  g_snprintf(person_parent_path, FILE_NAME_SIZE, "/opt/reid/frames/%d%02d%02d/run%s", current_time->tm_year+1900, current_time->tm_mon+1, current_time->tm_mday, run_count);
  // g_snprintf(face_parent_path, FILE_NAME_SIZE, "/opt/reid/thumb_nail/face_crops/%d%02d%02d/run%s", current_time->tm_year+1900, current_time->tm_mon+1, current_time->tm_mday, run_count);
  printf("Person Parent Path: %s \n", person_parent_path);
  // printf("Face Parent Path: %s \n", face_parent_path);
  gboolean ret = FALSE;
  NvDsPipeline *pipeline = &appCtx->pipeline;
  NvDsConfig *config = &appCtx->config;
  GstBus *bus;
