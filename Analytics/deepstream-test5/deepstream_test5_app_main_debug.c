...
/**
 * Recursively print all elements in a bin with their properties
 */
static void
print_element_info (GstElement *element, gint depth)
{
  ...
  element_type = GST_OBJECT_NAME (factory);

  // Print indentation based on depth
  for (i = 0; i < depth; i++)
    g_print ("  ");

  // Print element name and type
  g_print ("[%d] %s (type: %s)\n", depth, element_name, element_type);

  // Print element state
  GstState state, pending;
  GstStateChangeReturn state_ret = gst_element_get_state (element, &state, &pending, 0);
  if (state_ret == GST_STATE_CHANGE_SUCCESS || state_ret == GST_STATE_CHANGE_ASYNC) {
    const gchar *state_name = "UNKNOWN";
    switch (state) {
  ...
  // Print properties
  properties = g_object_class_list_properties (G_OBJECT_CLASS (GST_ELEMENT_GET_CLASS (element)), &n_properties);
  if (properties && n_properties > 0) {
    for (guint j = 0; j < n_properties; j++) {
      pspec = properties[j];
      
      // Skip certain internal properties for cleaner output
      if (g_str_has_prefix (pspec->name, "child-") ||
          g_str_has_prefix (pspec->name, "parent-") ||
          g_str_has_prefix (pspec->name, "name"))
        continue;

      g_value_init (&value, pspec->value_type);
      g_object_get_property (G_OBJECT (element), pspec->name, &value);
      str_value = gst_value_serialize (&value);
      if (str_value && strlen (str_value) > 0 && strlen (str_value) < 200) {
        for (i = 0; i < depth + 1; i++)
          g_print ("  ");
        g_print ("  %s = %s\n", pspec->name, str_value);
      }
      g_free (str_value);
      str_value = NULL;
      g_value_unset (&value);
...
/**
 * Print full pipeline structure with all elements and flags
 */
static void
print_pipeline_elements (AppCtx *appCtx)
{
  if (!appCtx || !appCtx->pipeline.pipeline) {
    g_print ("Pipeline is not available\n");
    return;
  }

  g_print ("\n");
  g_print ("========================================\n");
  g_print ("GSTREAMER PIPELINE ELEMENTS WITH FLAGS\n");
  g_print ("========================================\n");
  g_print ("\n");

  // Print main pipeline
  g_print ("MAIN PIPELINE:\n");
  print_element_info (appCtx->pipeline.pipeline, 0);

  // Print specific known elements for better visibility
  g_print ("\n");
  g_print ("KEY PIPELINE COMPONENTS:\n");
  g_print ("------------------------\n");

  if (appCtx->pipeline.multi_src_bin.bin) {
    g_print ("\nMULTI-SOURCE BIN:\n");
    print_element_info (appCtx->pipeline.multi_src_bin.bin, 0);
  }

  if (appCtx->pipeline.multi_src_bin.streammux) {
    g_print ("\nSTREAMMUX:\n");
    print_element_info (appCtx->pipeline.multi_src_bin.streammux, 0);
  }

  if (appCtx->pipeline.common_elements.primary_gie_bin.bin) {
    g_print ("\nPRIMARY GIE BIN:\n");
    print_element_info (appCtx->pipeline.common_elements.primary_gie_bin.bin, 0);
  }

  if (appCtx->pipeline.common_elements.tracker_bin.bin) {
    g_print ("\nTRACKER BIN:\n");
    print_element_info (appCtx->pipeline.common_elements.tracker_bin.bin, 0);
  }

  if (appCtx->pipeline.tiled_display_bin.bin) {
    g_print ("\nTILED DISPLAY BIN:\n");
    print_element_info (appCtx->pipeline.tiled_display_bin.bin, 0);
  }

  if (appCtx->pipeline.demuxer) {
    g_print ("\nDEMUXER:\n");
    print_element_info (appCtx->pipeline.demuxer, 0);
  }

  if (appCtx->pipeline.tiler_tee) {
    g_print ("\nTILER TEE:\n");
    print_element_info (appCtx->pipeline.tiler_tee, 0);
  }
  ...
  for (i = 0; i < MAX_SOURCE_BINS; i++) {
    if (appCtx->pipeline.instance_bins[i].bin) {
      g_print ("\nINSTANCE BIN [%d]:\n", i);
      print_element_info (appCtx->pipeline.instance_bins[i].bin, 0);
    }
  }

  g_print ("========================================\n");
  g_print ("END OF PIPELINE ELEMENTS\n");
  g_print ("========================================\n");
  g_print ("\n");
}
...
main (int argc, char *argv[])
{
  ...
  if (return_value != -1) {
    ...
    for (i = 0; i < num_instances; i++) {
      g_print ("\n=== Printing Pipeline Elements for Instance %d ===\n", i);
      print_pipeline_elements (appCtx[i]);
    }
  }
