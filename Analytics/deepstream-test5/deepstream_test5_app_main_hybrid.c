    ...
    NvDsBatchMeta * batch_meta, guint index)
{
  ...
  for (NvDsMetaList * l_frame = batch_meta->frame_meta_list; l_frame != NULL;
    ...
    // 1. Triton Output Logic for Class 80 (sgie-2)
    for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      NvDsObjectMeta *obj = (NvDsObjectMeta *) l_obj->data;
      if (obj->class_id == 80) {
        for (NvDsUserMetaList *l_user = obj->obj_user_meta_list; l_user != NULL; l_user = l_user->next) {
          NvDsUserMeta *user_meta = (NvDsUserMeta *)l_user->data;
          if (user_meta->base_meta.meta_type == NVDSINFER_TENSOR_OUTPUT_META) {
            NvDsInferTensorMeta *tensor_meta = (NvDsInferTensorMeta *)user_meta->user_meta_data;
            gchar *plate_text = NULL;
            gfloat plate_score = 0.0f;
            
            for (guint i = 0; i < tensor_meta->num_output_layers; i++) {
              NvDsInferLayerInfo *layer = &tensor_meta->output_layers_info[i];
              if (strcmp(layer->layerName, "TEXTS") == 0 && layer->buffer) {
                // Triton model now returns an array of int32_t (ASCII values)
                int32_t *chars = (int32_t *)layer->buffer;
                char text[32];
                for (int c = 0; c < 32; c++) {
                    text[c] = (char)chars[c];
                    if (text[c] == '\0') break;
                ...
                text[31] = '\0';
                if (strlen(text) > 0) {
                  plate_text = g_strdup(text);
                }
              }
              else if (strcmp(layer->layerName, "SCORES") == 0 && layer->buffer) {
                plate_score = ((float *)layer->buffer)[0];
              }
            }
            
            if (plate_text) {
              g_print("Plate Output (Class 80): %s (Conf: %f)\n", plate_text, plate_score);
              ...
              strncpy(label_info->result_label, plate_text, MAX_LABEL_SIZE - 1);
              label_info->result_prob = plate_score;
              nvds_add_label_info_meta_to_classifier(classifier_meta, label_info);
              nvds_add_classifier_meta_to_object(obj, classifier_meta);
              g_free(plate_text);
            }
          }
        }
    ...
    // 2. Parseq + CRAFT Logic for Classes 83 and 84 (sgie-0 + sgie-1)
    load_postproc_lib();
    ...
    for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      NvDsObjectMeta *obj = (NvDsObjectMeta *) l_obj->data;
      if (obj->class_id == 83 || obj->class_id == 84) {
    ...
    if (process_plate_fn && free_plate_result_fn) {
      for (int i = 0; i < num_plates; i++) {
        ...
        if (res) {
          g_print("Plate Output (Class %d): %s\n", plates[i]->class_id, res->text);
          NvDsObjectMeta *p = plates[i];
